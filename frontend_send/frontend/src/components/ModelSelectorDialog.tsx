...
export default function ModelSelectorDialog({
  ...
  function toggle(k: ModelKey) {
    setSelected((prev) => (prev.includes(k) ? prev.filter((x) => x !== k) : prev.length < 4 ? [...prev, k] : prev))
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-panel border border-panel rounded-lg p-4">
        ...
        <p className="text-sm text-muted mb-4">Selection order will define column order.</p>
          ...
          {ALL.map((m) => (
            <button
              ...
              className={`px-3 py-2 rounded border ${
                selected.includes(m.key) ? "border-[var(--color-primary)]" : "border-panel"
              } text-left`}
            >
